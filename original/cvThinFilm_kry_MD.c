...
#define NOUT         1200000                   /* number of output times */
...
int main()
{
  ...
  for (iout=1, tout = 10.0; iout <= NOUT; iout++) {
    ...
    if((umin-0.02) < 0.000001){ 
      printf("\n Born Repulsion acts at time %lf\n",t);
      break;
    }
    PrintOutput(cvode_mem, u, t);
...
static UserData AllocUserData(void)
{
  ...
  data->Jac = NULL;
  if(check_flag((void *)data,"malloc",2)) return(1);
  data->Jac = N_VNew_Serial(NEQ);
  if(check_flag((void *)data->Jac,"N_VNew_Serial",0)) return(1);
...
static void PrintOutput(void *cvode_mem, N_Vector u, realtype t)
{
  ...
  realtype A,sigma,curve1,curve2;
  udata = NV_DATA_S(u);
...
//  H0 =  1.00999;//at i = 30
  ...
  printf(" \n At time %f curve1 is %f and LSA  is %f  ",t,curve1,curve2);
